...
def _exact_discovery_hit(query: str, include_schema: bool = False) -> dict[str, Any] | None:
    ...
    if tool is not None:
        ...
        item = {
            ...
            "server": server,
            "description": (
                getattr(tool, "description", None) or record.get("summary") or ""
            ).strip(),
            "params": list((schema.get("properties") or {}).keys()),
            "score": 100.0,
            "match": "exact",
...
# invoke_tool.
# Compact operator seam: available in every router mode. Detailed product
# tools remain discoverable through find_tool/invoke_read_tool, so this adds
# one low-token natural-language entry point without expanding the default
# tool list.
...
async def inspect_network(
    ...
    """Review the network from a short request such as 'how does it look?'.

    Uses configured scoped defaults when identifiers are omitted, keeps
    Central and Mist identifiers separate, and returns a concrete scope
    question when no safe scope is available. Results include bounded evidence
    coverage and scoped drill-down actions. This read-only workflow never
    guesses a site mapping or performs writes.
    """
    ...
    if mist_site_id:
        args["mist_site_id"] = mist_site_id
