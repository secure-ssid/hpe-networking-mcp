...
AOS_S_TO_CX_MAPPINGS: list[MigrationFeatureMapping] = [
    MigrationFeatureMapping(
        ...
        target_syntax="interface 1/1/1-1/1/4\n  vlan trunk allowed 10\ninterface 1/1/5\n  vlan "
                      "access 20",
        explanation="AOS-S configures port membership inside the VLAN context ('vlan 10 tagged "
                    "1'). AOS-CX configures VLAN encapsulation directly under the interface "
                    "context ('vlan trunk allowed 10' or 'vlan access 20').",
        caveats=[
            "AOS-CX requires 'vlan trunk allowed' to include native VLAN if "
            "untagged traffic traverses the trunk."
        ],
    ),
    MigrationFeatureMapping(
        ...
        explanation="AOS-S assigns ports to trunk IDs ('trk1'). AOS-CX creates a logical LAG "
                    "interface ('interface lag 1') and adds member interfaces under each physical "
                    "port context.",
    ),
    MigrationFeatureMapping(
    ...
    MigrationFeatureMapping(
        ...
        target_syntax="vsf member 1\n  type jl658a\n  link 1 1/1/49,1/1/50\nvsf member 2\n  type "
                      ...
                      "mgmt",
        explanation="AOS-CX uses VSF (Virtual Switching Framework) with explicit 10G/25G/50G link "
                    "port bindings and MAD (Multi-Active Detection) via management port.",
    ),
    MigrationFeatureMapping(
        ...
        explanation="AOS-CX standardizes client port-access with multi-auth / concurrent 802.1X "
                    "and MAC-Auth under the interface block.",
    ),
]
...
CISCO_IOS_TO_CX_MAPPINGS: list[MigrationFeatureMapping] = [
    MigrationFeatureMapping(
        ...
        explanation="Cisco uses 'switchport mode access / access vlan'. AOS-CX uses 'vlan access "
                    "<id>' and 'vlan voice <id>'.",
    ),
    MigrationFeatureMapping(
        ...
        target_syntax="interface 1/1/1\n  no shutdown\n  vlan trunk native 1\n  vlan trunk allowed "
                      "10,20,30",
        explanation="Cisco uses 'switchport mode trunk / switchport trunk allowed vlan'. AOS-CX "
                    "uses 'vlan trunk allowed <list>'.",
    ),
    MigrationFeatureMapping(
        ...
        explanation="Cisco matches subnets via 'network <subnet> <wildcard>'. AOS-CX enables OSPF "
                    "directly on the interface / SVI ('ip ospf 1 area 0.0.0.0').",
    ),
]
    ...
    phases = [
        {
            ...
            "description": "Catalog switch models, serials, MAC addresses, transceivers, PoE power "
                           "budgets, and firmware versions.",
            "recommended_tools": ["list_devices", "get_device_health", "list_switch_interfaces"],
        },
        {
            "phase": "2. Architecture & VLAN Mapping",
            "description": "Design VSF stacking topology, Core-to-Access LAGs, Multi-VLAN "
                           "segmentation, and Dynamic Segmentation / EVPN-VXLAN.",
            "recommended_tools": ["get_switch_vlans", "lookup_hardware_specs"],
        },
        {
            "phase": "3. Central Cloud Group & Persona Provisioning",
            "description": "Define Central configuration group, assign personas (CAMPUS_AP, "
                           "ACCESS_SWITCH, AGG_SWITCH), and create port profiles.",
            "recommended_tools": ["list_groups", "create_vlan", "create_sw_port_profile"],
        },
        {
            "phase": "4. Cutover, Stacking & Validation",
            "description": "Execute VSF auto-stacking, verify LLDP neighbors, test 802.1X/MAC-Auth,"
                           " and validate gateway reachability.",
