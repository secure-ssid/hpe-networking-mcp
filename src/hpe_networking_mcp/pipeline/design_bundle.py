...
def _validate_quantity(quantity_raw: Any) -> int | None:
    """Validate quantity value.

    Must be a positive integer (int >= 1 or str representing positive int).
    Booleans, floats (including 1.0, 1.9, NaN, inf), negative numbers, zero,
    and non-integer strings/objects are rejected.
    """
    ...
    if isinstance(quantity_raw, int):
        return quantity_raw if quantity_raw >= 1 else None
    if isinstance(quantity_raw, str):
        try:
            val = int(quantity_raw.strip())
...
def _resolve_line_item(
    ...
    *,
    ...
    lookup_cache: dict[str, dict[str, Any]] | None = None,
) -> dict[str, Any]:
    ...
    sku_clean = sku_query.strip()
    if lookup_cache is not None and sku_clean in lookup_cache:
        lookup = lookup_cache[sku_clean]
    else:
        lookup = hardware_catalog.search(sku_clean, db_path=catalog_db_path)
        if lookup_cache is not None:
            lookup_cache[sku_clean] = lookup
    ...
    if lookup.get("match_type") != "exact_sku":
        return {
            ...
            "candidates": copy.deepcopy(lookup.get("results", [])),
            "field_labels": {
                "role": "operator_input",
                "quantity": "operator_input",
                "sku": "unknown",
    ...
    return {
        ...
        "source": copy.deepcopy(product["source"]),
        "lifecycle": copy.deepcopy(product["lifecycle"]),
        "field_labels": {
            "role": "operator_input",
            "quantity": "operator_input",
            "sku": "official",
...
def _resolve_jvd_reference(design_id: str, *, jvd_db_path: Path) -> dict[str, Any]:
    ...
    if not lookup.get("ok"):
        return {
            ...
            "warning": lookup.get("error", f"JVD design {design_id!r} could not be resolved"),
    ...
    *,
    ...
    for raw_item in line_items:
        if not isinstance(raw_item, dict):
            ...
            continue
        item = _resolve_line_item(
            raw_item, catalog_db_path=catalog_db_path, lookup_cache=lookup_cache
        )
