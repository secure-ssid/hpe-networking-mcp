...
hpe-networking-mcp registers **6,717 backend tools** when every generated surface and
guarded write is enabled: 6,705 platform API tools across nine vendor
...
make no vendor API call at all. Direct-all router mode adds seven more
router-native tools (present only in `direct` mode, not part of the backend
registry itself) for **6,724 client-visible tools total**. The recommended
...
Adding 577 curated platform tools yields the 6,705 platform API backend
total. Capability totals across the platform API catalog are 3,153 read, 165
...
| `rag-core` | 11 | 12 | Docs, exact API, advisory, lifecycle lookup, and skills/runbooks |
...
| **Platform API backend total** | **3,153** | **6,705** | Vendor-facing tools only -- see the note below |
...
| **Complete backend total** | **3,165** | **6,717** | Platform API total + `design-core` + `interop-core` |
...
6,717-tool registered catalog) and are included in the **complete**
backend total (6,717) reported everywhere else in this repository
...
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,724 | Full schema introspection and debugging |
...
`direct` mode's 6,724 = the complete 6,717 backend total + 7 router-native
