...
the local GLP preflight and SKU-catalog helpers. Direct-all router mode adds nineteen
router-native tools (present only in `direct` mode, not part of the backend
registry itself) for **6,751 client-visible tools total**. The recommended
...
The `default` count (35) is measured identically whether every toolset is
...
`direct` mode's 6,751 = the complete 6,732 backend total + 19 router-native
...
`mist_health`, `aos8_switches`, `aos8_ports`, `aos8_vlans`, `aos8_migration`,
`clearpass_endpoint`, `clearpass_active_session`, `clearpass_sessions`). The
other 13 `default`-mode wrapper names
