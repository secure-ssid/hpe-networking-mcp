...
hpe-networking-mcp registers **6,716 backend tools** when every generated surface and
guarded write is enabled: 6,704 platform API tools across nine vendor
...
registry itself) for **6,723 client-visible tools total**. The recommended
...
Adding 577 curated platform tools yields the 6,704 platform API backend
...
| **Platform API backend total** | **3,153** | **6,704** | Vendor-facing tools only -- see the note below |
...
| **Complete backend total** | **3,165** | **6,716** | Platform API total + `design-core` + `interop-core` |
...
6,716-tool registered catalog) and are included in the **complete**
backend total (6,716) reported everywhere else in this repository
...
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,723 | Full schema introspection and debugging |
...
`direct` mode's 6,723 = the complete 6,716 backend total + 7 router-native
