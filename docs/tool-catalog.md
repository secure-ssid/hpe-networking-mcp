...
hpe-networking-mcp registers **6,719 backend tools** when every generated surface and
guarded write is enabled: 6,706 platform API tools across nine vendor
...
`site-health` aggregator. Direct-all router mode adds seven more router-native
tools (present only in `direct` mode, not part of the backend registry itself)
for **6,726 client-visible tools total**. The recommended
...
Adding 579 curated platform tools yields the 6,706 platform API backend
total. Capability totals across the platform API catalog are 3,155 read, 165
diagnostic, 2,544 write, and 842 destructive. See
...
| `site-health` | 1 | 1 | Bounded cross-platform Central/Mist site-health aggregation with explicit availability states |
...
| `rag-core` | 14 | 14 | Docs, exact API, AOS-CX release comparison, advisory/lifecycle lookup, and skills/runbooks |
...
| **Platform API backend total** | **3,155** | **6,706** | Vendor-facing tools only -- see the note below |
...
| **Complete backend total** | **3,168** | **6,719** | Platform API total + `site-health` + `design-core` + `interop-core` |
...
6,719-tool registered catalog) and are included in the **complete**
backend total (6,719) reported everywhere else in this repository
...
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,726 | Full schema introspection and debugging |
...
`direct` mode's 6,726 = the complete 6,719 backend total + 7 router-native
