...
the local GLP preflight diagnostic. Direct-all router mode adds twelve more
router-native tools (present only in `direct` mode, not part of the backend
registry itself) for **6,741 client-visible tools total**. The recommended
...
`direct` mode's 6,741 = the complete 6,729 backend total + 12 router-native
