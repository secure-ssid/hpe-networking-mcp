...
hpe-networking-mcp registers **6,731 backend tools** when every generated surface and
guarded write is enabled: 6,712 platform API tools across nine vendor
surfaces (Aruba Central, GreenLake Platform, ClearPass, Juniper Mist,
Apstra, ArubaOS 8, EdgeConnect, UXI, Axis Atmos Cloud) plus 8 credential-free
...
the local GLP preflight and SKU-catalog helpers. Direct-all router mode adds twelve more
router-native tools (present only in `direct` mode, not part of the backend
registry itself) for **6,743 client-visible tools total**. The recommended
...
The nine committed generated manifests contain **6,145 operations** (6,128
register as active generated tools; 17 are intentionally excluded — see below).
Adding 584 curated platform tools yields the 6,712 platform API backend
total. Capability totals across the platform API catalog are 3,160 read, 165
...
| `rag-core` | 15 | 16[^rag-provenance] | Docs, exact API, AOS-CX release comparison, advisory/lifecycle lookup, and skills/runbooks |
...
| **Platform API backend total** | **3,160** | **6,712** | Vendor-facing tools only -- see the note below |
...
| **Complete backend total** | **3,177** | **6,731** | Platform API total plus the non-platform backends itemized in [`docs/project-facts.json`](project-facts.json) |

[^glp-preflight]: `docs/project-facts.json` reports `tools.by_server["glp-core"]` as
    1,015 -- the 1,013 vendor-facing tools plus the local `glp_preflight`
...
[^rag-provenance]: `docs/project-facts.json` reports `tools.by_server["rag-core"]`
    as 16 -- the 15 tools counted in the platform API total plus local
    `corpus_provenance`, which is listed separately below because it reads only
    local artifacts and is excluded from the platform API total.
...
6,731-tool registered catalog) and are included in the **complete**
backend total (6,731) reported everywhere else in this repository
...
17 operations total are excluded this way, so 6,128 of the 6,145 manifest
...
| `default` | 27 | Router convenience wrappers (`list_sites`, `ask_docs`, `search_hardware_catalog`, `compare_hardware`, `mist_clients`, etc.) |
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,743 | Full schema introspection and debugging |

The `default` count (27) is measured identically whether every toolset is
...
`direct` mode's 6,743 = the complete 6,731 backend total + 12 router-native
