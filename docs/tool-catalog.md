...
hpe-networking-mcp registers **6,715 backend tools** when every generated surface and
guarded write is enabled: 6,703 platform API tools across nine vendor
...
registry itself) for **6,722 client-visible tools total**. The recommended
...
Adding 576 curated platform tools yields the 6,703 platform API backend
total. Capability totals across the platform API catalog are 3,152 read, 165
...
6,715-tool registered catalog) and are included in the **complete**
backend total (6,715) reported everywhere else in this repository
...
`direct` mode's 6,722 = the complete 6,715 backend total + 7 router-native
