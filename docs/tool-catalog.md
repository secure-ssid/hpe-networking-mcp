...
hpe-networking-mcp registers **6,729 backend tools** when every generated surface and
...
the local GLP preflight diagnostic. Direct-all router mode adds nineteen more
router-native tools (present only in `direct` mode, not part of the backend
registry itself) for **6,748 client-visible tools total**. The recommended
...
The nine committed generated manifests contain **6,144 operations** (6,128
...
| **Complete backend total** | **3,176** | **6,729** | Platform API total plus the non-platform backends itemized in [`docs/project-facts.json`](project-facts.json) |
...
6,729-tool registered catalog) and are included in the **complete**
backend total (6,729) reported everywhere else in this repository
...
17 operations total are excluded this way, so 6,128 of the 6,144 manifest
...
| `default` | 32 | Router convenience wrappers (`list_sites`, `ask_docs`, `mist_clients`, etc.) |
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,748 | Full schema introspection and debugging |

The `default` count (32) is measured identically whether every toolset is
...
`direct` mode's 6,748 = the complete 6,729 backend total + 19 router-native
