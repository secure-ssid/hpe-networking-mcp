...
hpe-networking-mcp registers **6,732 backend tools** when every generated surface and
guarded write is enabled: 6,713 platform API tools across nine vendor
...
registry itself) for **6,744 client-visible tools total**. The recommended
...
Adding 584 curated platform tools yields the 6,713 platform API backend
total. Capability totals across the platform API catalog are 3,161 read, 165
...
| **Complete backend total** | **3,177** | **6,732** | Platform API total plus the non-platform backends itemized in [`docs/project-facts.json`](project-facts.json) |
...
6,732-tool registered catalog) and are included in the **complete**
backend total (6,732) reported everywhere else in this repository
...
| `default` | 28 | Router convenience wrappers (`list_sites`, `ask_docs`, `search_hardware_catalog`, `compare_hardware`, `mist_clients`, etc.) |
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,744 | Full schema introspection and debugging |

The `default` count (28) is measured identically whether every toolset is
...
`direct` mode's 6,744 = the complete 6,732 backend total + 12 router-native
