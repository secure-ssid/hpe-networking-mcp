...
hpe-networking-mcp registers **6,722 backend tools** when every generated surface and
guarded write is enabled: 6,708 REST/OpenAPI platform API tools across nine vendor
surfaces (Aruba Central, GreenLake Platform, ClearPass, Juniper Mist,
Apstra, ArubaOS 8, EdgeConnect, UXI, Axis Atmos Cloud) plus the local-only
`glp_preflight` diagnostic, 7 credential-free
`design-core` diagram tools and 5 credential-free `interop-core` tools that
make no vendor API call at all. The protocol-only Central Streaming collector
adds one vendor-facing backend tool outside the REST/OpenAPI total. Direct-all
router mode adds seven more router-native tools (present only in `direct` mode,
not part of the backend registry itself) for **6,729 client-visible tools total**.
The recommended
...
Adding 581 curated platform tools yields the 6,708 platform API backend
total. The separate `central-streaming` protocol-only backend contributes one
additional tool, and the local-only `glp_preflight` diagnostic contributes
another non-vendor tool. The complete backend total is therefore 6,722 after
including the seven `design-core` and five `interop-core` tools.
Capability totals across the REST/OpenAPI platform API catalog are 3,156 read, 165
...
| `central-monitoring` | 75 | 90 | Health, inventory, topology, BSSID mappings, applications, onboarding, AP tunnels, config health, notification rules, guarded Central GET, reports, and per-device/site troubleshooting plans |
...
| `central-streaming` | 1 | 1 | Read-only Central WSS event collection for AP Monitoring, Audit Trail, Geofence, Location, and Location Analytics; protocol-only, Advanced subscription required |
| `glp-core` | 573 | 1,014 | Current devices, grouping, subscriptions, auto-subscription settings, users, Audit Logs v2beta1, workspaces, reporting, service catalog, RBAC role-assignment/scope-group lifecycle, identity user lifecycle, events/webhooks/deliveries, locations/tags, SCIM users/groups/membership, region-aware Compute Ops Management/Storage Fleet/Block Storage/Virtualization/Backup & Recovery/Data Services reads and guarded writes with explicit family/host metadata, read-only cross-resource reconciliation, regional async-operation polling and cancellation, and local-only `glp_preflight` readiness diagnostics (108 curated + 906 active generated; `HPE_MCP_GLP_GENERATED_TOOLS=1` to expand) |
| `rag-core` | 12 | 12 | Docs, exact API, advisory, lifecycle lookup, API-family summaries, and skills/runbooks |
...
| **Platform API backend total** | **3,156** | **6,708** | REST/OpenAPI vendor-facing tools only -- see the note below |
| **Protocol-only backend total** | **1** | **1** | Central Streaming WSS collector; not represented in OpenAPI manifests |
...
| **Complete backend total** | **3,170** | **6,722** | Platform API total + protocol-only Central Streaming + local-only GLP preflight + `design-core` + `interop-core` |
...
6,722-tool registered catalog) and are included in the **complete**
backend total (6,722) reported everywhere else in this repository
...
| `direct` + `HPE_MCP_TOOLSETS=all` | 6,729 | Full schema introspection and debugging |
...
`direct` mode's 6,729 = the complete 6,722 backend total + 7 router-native
