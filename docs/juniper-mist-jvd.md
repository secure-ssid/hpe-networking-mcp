...
- `quantity` enforces strict positive integer semantics (`quantity >= 1` or integer
  strings like `"2"`, `"+2"`, and `"1_000"`). Booleans, floats (including `1.0`, `1.9`, `NaN`, `inf`),
  negative numbers, zero, and non-integer strings are rejected as explicit unresolved
  items without silent rounding or truncation.
- Hardware catalog lookups for line items with identical SKUs reuse request-local
  cached results within a single `build_design_bundle` call (bounded by `MAX_LINE_ITEMS`),
  avoiding redundant SQLite queries without global or cross-request caching.
  Resolved source/lifecycle metadata and unresolved candidate records remain deeply
  independent between rows.
...
- `topology` reuses the exact same `design_lib.model.parse_model` validation
  ...
  bundle's `topology` field can be handed directly to those exporters (including
  NeXt UI HTML preview generation with offline static SVG and text/table fallback). An
