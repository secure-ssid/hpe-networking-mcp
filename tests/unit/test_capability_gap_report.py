...
def test_capability_report_counts_and_classification_are_deterministic():
    ...
    assert first == second
    assert sum(row["generated"] for row in rows) == 6145
    assert sum(row["registered_generated"] for row in rows) == 6128
    assert sum(row["curated"] for row in rows) == 585
    assert sum(row["registered"] for row in rows) == 6713
